...
def _extract_post_text(item):
    actualPosts = item.find_all(class_= "_5rgt _5nk5 _5msi" )
    print (len (actualPosts) )
    text = ""
    if actualPosts:
...
def _extract_link(item):
    postLinks = item.find_all(class_="_52jc _5qc4 _78cz _24u0 _36xo") #"_6ks")
    link = ""
    for postLink in postLinks:
        link = postLink.find('a', recursive = False).get('href')
    return link
...
def _extract_html(bs_data):
    ...
    k = bs_data.find_all(class_="_55wo _gui")
    postBigDict = list()
